...
<?if (!$arSection && $arParams['SET_STATUS_404'] !== 'Y'):?>
	<div class="rounded-3xl border border-amber-200 bg-amber-50 px-6 py-5 text-amber-900"><?=GetMessage("SECTION_NOTFOUND")?></div>
<?elseif (!$arSection && $arParams['SET_STATUS_404'] === 'Y'):?>
	<?CScorp::goto404Page();?>
<?else:?>
	<div class="space-y-10">
		<?if (!$arSubSections && !$itemsCnt):?>
			<div class="rounded-3xl border border-amber-200 bg-amber-50 px-6 py-5 text-amber-900"><?=GetMessage("SECTION_EMPTY")?></div>
		<?endif;?>
		<?if ($arSubSections):?>
