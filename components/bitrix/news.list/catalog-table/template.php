	...
	<?if($arResult["ITEMS"]):?>
		...
		<ul class="container mx-auto grid gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-4" itemscope itemtype="https://schema.org/ItemList">
			<?foreach($arResult["ITEMS"] as $i => $arItem):?>
				<?
if($arItem["DISPLAY_PROPERTIES"]["TECH_TYPE"]["VALUE_XML_ID"]) $arItem['DETAIL_PAGE_URL'] = $arItem['DETAIL_PAGE_URL'].'&'.$arItem["DISPLAY_PROPERTIES"]["TECH_TYPE"]["VALUE_XML_ID"];
	...
	<?if(is_array($arResult["SECTION"]["PATH"])):?>
		...
		<?if(isset($arResult["SECTION_DESCRIPTION"]) && (strlen($arResult["SECTION_DESCRIPTION"]) >  0) && strpos($_SERVER["REQUEST_URI"], "PAGEN") === false):?>
			<div class="cat-desc"><hr style="<?=(strlen($arResult['NAV_STRING']) && $arParams['DISPLAY_BOTTOM_PAGER'] ? 'margin-top:20px;' : 'border-color:transparent;margin-top:0;margin-bottom:10px;')?>" /><?=$arResult["SECTION_DESCRIPTION"]?></div>
		<?endif;?>
	<?endif;?>
</div>
<?$frame->end();?>
